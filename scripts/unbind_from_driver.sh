...
# For AMD/ROCm GPUs, attempt to acquire the unbindLock within the retries specified before unbinding.

acquire_unbind_lock()
{
   local gpu=$1
   local lock_retries=5
   local unbind_lock_file="/proc/driver/amdgpu/gpus/$gpu/unbindLock"
   local unbind_lock=0
   local attempt=1
...
unbind_from_driver()
{
   ...
   if [ "${existing_driver_name}" == "amdgpu" ]; then
